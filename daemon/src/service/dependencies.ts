...
export function checkDependencies() {
  ...
  dependencies.forEach((path) => {
    if (!fs.existsSync(path)) {
      throw new Error(`ElementsPanel requires additional dependencies to run. Please download the files suitable for your system architecture from the following locations:\n1. https://github.com/MCSManager/PTY/releases\n2. https://github.com/MCSManager/Zip-Tools/releases`);
    }
  });
}
